package io.recheck.uuidprotocol.user.auth.x509.filter;
...
public abstract class AbstractPreAuthenticatedProcessingFilterCustom extends GenericFilterBean
    ...
    private RequestMatcher requiresAuthenticationRequestMatcher = new AbstractPreAuthenticatedProcessingFilterCustom.PreAuthenticatedProcessingRequestMatcher();
    ...
    protected void unsuccessfulAuthentication(HttpServletRequest request, HttpServletResponse response,
        ...
        this.logger.debug("Cleared x509 context due to exception", failed);
    ...
    private class PreAuthenticatedProcessingRequestMatcher implements RequestMatcher {
        ...
        public boolean matches(HttpServletRequest request) {
            Authentication currentUser = AbstractPreAuthenticatedProcessingFilterCustom.this.securityContextHolderStrategy
                    .getContext()
                    .getAuthentication();
            if (currentUser == null) {
                return true;
            }
            if (!AbstractPreAuthenticatedProcessingFilterCustom.this.checkForPrincipalChanges) {
                return false;
            }
            if (!principalChanged(request, currentUser)) {
                return false;
            }
            AbstractPreAuthenticatedProcessingFilterCustom.this.logger
                    .debug("Pre-authenticated principal has changed and will be reauthenticated");
            if (AbstractPreAuthenticatedProcessingFilterCustom.this.invalidateSessionOnPrincipalChange) {
                AbstractPreAuthenticatedProcessingFilterCustom.this.securityContextHolderStrategy.clearContext();
                HttpSession session = request.getSession(false);
                if (session != null) {
                    AbstractPreAuthenticatedProcessingFilterCustom.this.logger.debug("Invalidating existing session");
                    session.invalidate();
                    request.getSession();
                }
            }
