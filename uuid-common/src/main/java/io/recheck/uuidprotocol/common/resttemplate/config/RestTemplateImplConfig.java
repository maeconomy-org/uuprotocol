...
public class RestTemplateImplConfig {

    @SneakyThrows
    public RestTemplateImpl buildSSLRestTemplate(SSLContextSpec sslContextSpec, ServerSpec serverSpec) {
        SSLContext sslContext;

        if (StringUtils.hasText(sslContextSpec.getKeyStoreAlias())) {
            sslContext = SSLContexts.custom()
                    .loadTrustMaterial(ResourceUtils.getFile(sslContextSpec.getTrustStoreFile()), sslContextSpec.getTruststorePassword().toCharArray())
            ...
            sslContext = SSLContexts.custom()
                    .loadTrustMaterial(ResourceUtils.getFile(sslContextSpec.getTrustStoreFile()), sslContextSpec.getTruststorePassword().toCharArray())
                    .loadKeyMaterial(ResourceUtils.getFile(sslContextSpec.getKeyStoreFile()), sslContextSpec.getKeystorePassword().toCharArray(), sslContextSpec.getKeystorePassword().toCharArray())
                    .build();
        }
